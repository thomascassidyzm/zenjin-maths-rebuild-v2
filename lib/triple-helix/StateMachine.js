...
// Import the Zustand stitch adapter
const zustandStitchAdapter = require('./zustand-stitch-adapter');

// Import utility to load content from the Zustand store
const { loadContentIntoStateMachine, createEmergencyStitch } = require('./zustand-stitch-adapter');

class StateMachine {
  constructor(initialState = {}) {
    // CRITICAL FIX: Implement robust userId validation and recovery
    let userId = initialState.userId;
    
    // Handle null/undefined/empty userId with recovery mechanisms
    if (!userId || userId === '') {
      console.error('CRITICAL ERROR: Empty or missing userId provided to StateMachine, attempting recovery');
      
      // Recovery mechanism 1: Try to find userId in localStorage
      if (typeof window !== 'undefined') {
        try {
          const storedUserId = localStorage.getItem('zenjin_user_id');
          if (storedUserId && storedUserId !== '') {
            console.log(`CRITICAL RECOVERY: Found userId ${storedUserId} in localStorage`);
            userId = storedUserId;
          } else {
            // Recovery mechanism 2: Check for Supabase auth session
            try {
              const supabaseSession = localStorage.getItem('sb-ggwoupzaruiaaliylxga-auth-token');
              if (supabaseSession) {
                const parsedSession = JSON.parse(supabaseSession);
                if (parsedSession && parsedSession.user && parsedSession.user.id) {
                  console.log(`CRITICAL RECOVERY: Found user ID ${parsedSession.user.id} in Supabase session`);
                  userId = parsedSession.user.id;
                }
              }
            } catch (sessionErr) {
              console.error('Error recovering from Supabase session:', sessionErr);
            }
          }
        } catch (e) {
      ...
      // If all recovery fails, use anonymous ID
      if (!userId || userId === '') {
        console.log('CRITICAL RECOVERY: All recovery methods failed, using anonymous fallback');
        userId = 'anonymous';
      }
    }
    
    // Log userId for debugging
    console.log(`StateMachine initializing with userId: ${userId}`);
    
    // Store this userId in localStorage for future recovery
    if (typeof window !== 'undefined' && userId && userId !== 'anonymous') {
      try {
        localStorage.setItem('zenjin_user_id', userId);
        console.log(`Saved userId ${userId} to localStorage for resilience`);
      } catch (e) {
    ...
    // Override with provided state, but ensure userId is preserved
    if (initialState && Object.keys(initialState).length > 0) {
      console.log('Initializing StateMachine with provided state');
      
      // Create a new state object with our validated userId
      this.state = {
        ...this.state,
        ...initialState,
      ...
      // Double-check userId is not empty after merge
      if (!this.state.userId || this.state.userId === '') {
        console.error('CRITICAL ERROR: userId is empty after state merge, using fallback');
    ...
    if (isAnonymousUser) {
      console.log(`Anonymous user detected (userId: ${this.state.userId}) - preserving localStorage state when available`);
      
      // CRITICAL FIX: Check if there's an actual anonymousId in localStorage, which might be different
      const localStorageAnonymousId = localStorage.getItem('anonymousId');
      if (localStorageAnonymousId && localStorageAnonymousId !== this.state.userId) {
        console.log(`IMPORTANT: Found anonymousId in localStorage (${localStorageAnonymousId}) which differs from state.userId (${this.state.userId})`);
      }
      
      // For anonymous users, check localStorage first as they don't have server state
      if (typeof window !== 'undefined') {
        try {
          // First check using the anonymousId from localStorage if available
          let savedState = null;
          const anonymousId = localStorage.getItem('anonymousId');
          
          if (anonymousId) {
            savedState = localStorage.getItem(`triple_helix_state_${anonymousId}`);
            console.log(`Checking localStorage with anonymousId (${anonymousId}): ${savedState ? 'FOUND' : 'NOT FOUND'}`);
          }
          
          // If not found with anonymousId, try with state.userId
          if (!savedState) {
            savedState = localStorage.getItem(`triple_helix_state_${this.state.userId}`);
            console.log(`Checking localStorage with state.userId (${this.state.userId}): ${savedState ? 'FOUND' : 'NOT FOUND'}`);
          }
          ...
          if (savedState) {
            ...
            try {
              ...
              // We'll log additional information to help with debugging
              console.log(`Validating state format:`, {
                tubes: parsedState?.tubes ? Object.keys(parsedState.tubes).length : 0,
                activeTube: parsedState?.activeTubeNumber || 'none'
              });
              
              // Simple and direct validation - no complex logic needed
              const isValidState = 
                parsedState && 
                parsedState.tubes && 
    ...
    // Load content from the Zustand store with correct positions
    // This ensures all stitches are available and properly positioned
    setTimeout(async () => {
  ...
  async handleStitchCompletion(threadId, stitchId, score, totalQuestions) {
    ...
    if (isPerfectScore) {
      ...
      await this.advanceStitchInTube(tubeNumber);
    } else {
   ...
   * @returns {Promise<Object|null>} Promise resolving to the new current stitch or null if error
   */
  async advanceStitchInTube(tubeNumber) {
    const tube = this.state.tubes[tubeNumber];
    ...
    if (position1Stitches.length > 0) {
      ...
      if (candidateStitches.length > 0) {
        ...
        // Load all stitches from Zustand store and ensure they're properly positioned
        console.log(`RECOVERY: Loading all stitches from Zustand store for tube ${tubeNumber}`);

        // Load all stitches for this thread
        const allThreadStitches = await zustandStitchAdapter.getAllStitchesForThread(tubeNumber, currentStitch.threadId);
        
        if (allThreadStitches.length > 0) {
          console.log(`RECOVERY: Loaded ${allThreadStitches.length} stitches from Zustand store`);
          
          // Add new stitches to the tube (if they don't already exist)
          for (const stitch of allThreadStitches) {
            if (!tube.stitches.some(s => s.id === stitch.id)) {
              tube.stitches.push(stitch);
              console.log(`RECOVERY: Added stitch ${stitch.id} to tube ${tubeNumber}`);
            }
          }
          
          // Normalize all stitch positions to ensure proper ordering
          this._normalizeStitchPositions(tube, true);
          
          // Find the stitch at position 1 (which should be the next stitch)
          ...
          if (position1Stitch) {
            ...
            positionRegistry.add(0); // Current stitch at position 0
            positionRegistry.add(1); // Next stitch at position 1
            
            // Save state
            this._saveState();
            
            // Return current stitch to avoid skipping ahead
            return this.getCurrentStitch();
          }
        }
        ...
        const allStitches = await zustandStitchAdapter.getAllStitchesForThread(tubeNumber, currentStitch.threadId);
    ...
    // CRITICAL FIX: Log details about the new active stitch
    console.log(`STITCH ADVANCEMENT: New active stitch is ${nextStitch.id} with ${nextStitch.questions?.length || 0} questions`);
    console.log(`STITCH ADVANCEMENT: Thread ID is ${nextStitch.threadId}, tube number is ${tubeNumber}`);
