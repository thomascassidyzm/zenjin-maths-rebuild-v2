...
// Import utility to load all bundled stitches with correct positions
// This fixes the stitch advancement issue by ensuring all stitches are properly positioned
...
class StateMachine {
  constructor(initialState = {}) {
    ...
    // CRITICAL FIX: Load all bundled stitches with correct positions
  ...
  advanceStitchInTube(tubeNumber) {
    ...
    if (position1Stitches.length > 0) {
      ...
      if (candidateStitches.length > 0) {
        ...
        // SIMPLE FIX: Load all bundled stitches and ensure they're properly positioned
        console.log(`RECOVERY: Loading all stitches from bundled content for tube ${tubeNumber}`);
        ...
        if (allThreadStitches.length > 0) {
          console.log(`RECOVERY: Loaded ${allThreadStitches.length} stitches from bundled content`);
