...
function updateConfigWithNewStitches(config, tubeNumber, threadId, newStitches) {
  ...
  // CRITICAL FIX: Ensure proper position initialization to enable stitch advancement
  if (tube.activeStitches.length >= 2) {
    // The first stitch should be at position 0
    if (tube.activeStitches[0].orderNumber !== 0) {
      tube.activeStitches[0].orderNumber = 0;
    }
    
    // The second stitch should be at position 1 to ensure there's always a next stitch
    if (tube.activeStitches[1].orderNumber !== 1) {
      tube.activeStitches[1].orderNumber = 1;
    }
    
    // Make sure positions are consecutive from 0 to N for remaining stitches
    for (let i = 2; i < tube.activeStitches.length; i++) {
      tube.activeStitches[i].orderNumber = i;
    }
    
    console.log(`CRITICAL FIX: Initialized ${tube.activeStitches.length} stitches with sequential positions for tube ${tube.number}`);
  }
}
