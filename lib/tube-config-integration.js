...
export async function initializeTubeCycler(user, options = {}) {
  try {
    ...
    // Immediate buffer check to load content for any empty tubes
    setTimeout(() => {
...
function createOfflineFirstContentManager() {
  return {
    ...
    async getContent(stitchId) {
      return await offlineFirstContentBuffer.getStitch(stitchId);
    },
    
    // Add to cache - this is a no-op since the offline-first buffer manages its own cache
    addToCache(stitches) {
      // Nothing needed since offline-first buffer handles caching
    },
