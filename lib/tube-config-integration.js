...
// Import core fixes to address initialization issues
import { validateAndFixInitialization, validateAndFixTubeStructure, ensureValidContentManager } from './core-fix.js';
...
export async function initializeTubeCycler(user, options = {}) {
  try {
    ...
    // Apply core fix - validate and fix tube structure
    ...
    if (config.tubes) {
      try {
        ...
        console.error('Error during tube validation:', validationError);
        // If validation fails, create a default configuration
        config = createDefaultConfiguration();
      }
    }
    ...
    let adapter = new TubeCyclerAdapter({
    ...
    // Apply core fix - ensure adapter has a valid content manager
