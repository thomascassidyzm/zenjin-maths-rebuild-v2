...
export async function initializeTubeCycler(user, options = {}) {
  try {
    ...
    // CRITICAL FIX: Preload all bundled stitches in each tube to ensure stitch advancement works
    // This ensures there's always a next stitch to advance to
    setTimeout(() => {
      // Preload all bundled stitches for each tube (10 per tube)
      for (let tubeNumber = 1; tubeNumber <= 3; tubeNumber++) {
        ...
        // Load all 10 bundled stitches for this tube
        // We start with index 0 and load 10 stitches
        console.log(`CRITICAL FIX: Preloading all 10 bundled stitches for tube ${tubeNumber}`);
