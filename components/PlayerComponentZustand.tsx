...
import { 
  ...
  getLegacyState,
  saveStateToServer
} from '../lib/store/stateAdapter';
import { useMigratedComponent } from '../lib/store/migrationUtils';

/**
 * PlayerComponentZustand - Enhanced with Zustand state management
 * 
 ...
 * 4. Zustand store for state persistence
 * 5. Database storage at session end for authenticated users
 */
export default function PlayerComponentZustand({ user }) {
  // Mark this component as migrated to Zustand
  useMigratedComponent('PlayerComponentZustand');
  ...
  // Use Zustand store state for current tube
  const currentTube = useAppStore(state => 
    state.tubeState?.activeTube || state.tubeState?.activeTubeNumber || 1
  );
  
  // Use Zustand store for updating tube state
  const setActiveTube = useAppStore(state => state.setActiveTube);
  
  // Initialize on component mount - with immediate content
  useEffect(() => {
    async function initialize() {
      try {
        console.log('Initializing TubeCycler with Zustand...');
        ...
        // Get the initial state and update Zustand store
        if (adapter) {
          const initialState = adapter.getState();
          updateStoreFromLegacyState(initialState);
          
          // Set active tube in Zustand store
          setActiveTube(initialState.currentTube || 1);
        }
  ...
  const handleStateChange = (state) => {
    ...
    // Update Zustand store with the latest state
    updateStoreFromLegacyState(state);
  };
  ...
  const updateUIAfterCompletion = (adapter) => {
    ...
    // Update active tube in Zustand store
    setActiveTube(newState.currentTube || 1);
  };
  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        // Get the latest state from tubeCycler and update Zustand store
        const latestState = tubeCycler.getState();
        updateStoreFromLegacyState(latestState);
      ...
      // For anonymous users: save to localStorage and generate local summary
        ...
        // Save to localStorage via tubeCycler
        saveToLocalStorage(null, tubeCycler.getState());
        
        // Update Zustand store and sync there too
        const latestState = tubeCycler.getState();
        updateStoreFromLegacyState(latestState);
        await saveStateToServer();
      ...
      alert('Error saving progress. Your progress is still saved locally.');
      setIsSaving(false);
    }
  };
  ...
  // Get points from Zustand store
  const points = useAppStore(state => state.learningProgress?.points || { session: 0, lifetime: 0 });
  ...
  return (
    <div className="player-container">
      ...
      <div className="points-display">
        <p>Session Points: {points.session}</p>
        <p>Total Points: {points.lifetime}</p>
      </div>
      
      {/* Tube indicator from Zustand store */}
      <div className="tube-indicator">
        <p>Current Tube: {currentTube}</p>
      </div>
