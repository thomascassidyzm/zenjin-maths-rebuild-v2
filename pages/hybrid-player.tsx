 ...
 * Hybrid Player Page
 * 
 * This page demonstrates the hybrid approach that combines:
 * 1. Zustand-based content fetching
 * 2. Existing MinimalDistinctionPlayer for UI
 * 
 * This approach gives us the best of both worlds:
 * - Consistent content loading using Zustand
 * - Reliable UI with the existing player component
 */
...
export default function HybridPlayerPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-800 to-purple-900">
      <Head>
        <title>Hybrid Player - Zenjin Maths</title>
        <meta name="description" content="Hybrid Zustand + MinimalDistinctionPlayer for Zenjin Maths" />
      </Head>
      
      <main className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold mb-2 text-white text-center">Hybrid Player</h1>
        <p className="text-white text-opacity-80 text-center mb-8">
          Combining Zustand content fetching with MinimalDistinctionPlayer
        </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            {!isInitialized ? (
              ...
              <div className="bg-white/5 backdrop-blur-lg rounded-xl overflow-hidden">
                <ZustandContentProvider
                  ...
                  sessionTotalPoints={useZenjinStore(state => state.learningProgress?.evoPoints || 0)}
                />
              </div>
            ) : (
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 text-white">
            ...
            <div className="bg-indigo-800/30 border border-indigo-500/30 p-4 rounded-xl mb-6">
              <div className="text-center">
                <h3 className="text-white/70 text-sm uppercase tracking-wide mb-1">Total Points</h3>
                <p className="text-3xl font-bold bg-gradient-to-r from-blue-400 to-indigo-300 text-transparent bg-clip-text">
                  {formatPoints(useZenjinStore(state => state.learningProgress?.evoPoints || 0))}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <pre className="bg-gray-900/50 p-3 rounded-md text-xs overflow-auto max-h-28">
                  {formatJson(userInfo)}
                </pre>
              </div>
              
              <div>
                <h3 className="font-medium mb-1">Active Stitch</h3>
                <div className="bg-gray-900/50 p-3 rounded-md font-mono text-sm">
                  {activeStitchId || 'None'}
                </div>
              </div>
              
              <div>
                <h3 className="font-medium mb-1">Content Collection</h3>
                <div className="bg-gray-900/50 p-3 rounded-md">
                  <p className="font-mono text-sm">
                    {Object.keys(useZenjinStore(state => state.contentCollection?.stitches || {})).length} stitches cached
                  </p>
                </div>
              </div>
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <button
                  onClick={() => useZenjinStore.getState().syncToServer()}
                  className="px-3 py-2 bg-purple-600 hover:bg-purple-500 rounded-md text-sm"
                >
                  Sync to Server
                </button>
              </div>
